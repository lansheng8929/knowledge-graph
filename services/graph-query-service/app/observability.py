...
- 结构化日志：JSON 格式，便于 Loki 收集
- OpenTelemetry Trace：OTLP exporter，惰性初始化（未装依赖/未配 collector 时静默跳过）
- 审计通道占位：Phase 4 接入不可篡改审计 sink
"""
...
def init_tracing(service_name: str) -> bool:
    ...
    返回 True 表示已启用；未配置 OTEL_EXPORTER_OTLP_ENDPOINT 或未安装依赖时返回 False（静默跳过）。
    全链路需要 docker 起 OTel Collector / Jaeger / Tempo（用户手动部署）。
    """
