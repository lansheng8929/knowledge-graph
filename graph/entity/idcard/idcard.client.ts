...
import type { IdCardNodeType } from "./idcard.type"

export const createIdcardEntity: EntityCreator = () => {
  const imageCache = new Map<string, HTMLImageElement>()

  return {
    renderNodeCanvasObject: ({ node, ctx, globalScale, style }) => {
      const { x = 0, y = 0 } = node
      const { idCard, pageIndex, pageSize, count } =
        (node.data as IdCardNodeType["data"]) ?? {}

      const iconPath = `/api/v1/connValueType-icon?key=idcard`
