...
import type { Ipv6NodeType } from "./ipv6.type"

export const createIpv6Entity: EntityCreator = () => {
  const imageCache = new Map<string, HTMLImageElement>()

  return {
    renderNodeCanvasObject: ({ node, ctx, globalScale, style }) => {
      const { x = 0, y = 0 } = node
      const { ip, country, location, pageIndex, pageSize, count } =
        (node.data as Ipv6NodeType["data"]) ?? {}

      const iconPath = `/api/v1/flag-icon?national=${country}`
