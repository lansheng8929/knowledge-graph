...
import type { BankCardNodeType } from "./bankcard.type"

export const createBankCardEntity: EntityCreator = () => {
  const imageCache = new Map<string, HTMLImageElement>()

  return {
    renderNodeCanvasObject: (node, ctx, globalScale, style) => {
      const { x = 0, y = 0 } = node
      const { bankCard, label, pageIndex, pageSize, count } =
        (node.data as BankCardNodeType["data"]) ?? {}

      const iconPath = `/api/v1/connValueType-icon?key=bankcard`
