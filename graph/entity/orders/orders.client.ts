...
import type { OrderNodeType } from "./orders.type"

export const createOrdersEntity: EntityCreator = () => {
  const imageCache = new Map<string, HTMLImageElement>()

  return {
    renderNodeCanvasObject: ({ node, ctx, globalScale, style }) => {
      const { x = 0, y = 0 } = node
      const { label, orderNumber, pageIndex, pageSize, count } =
        (node.data as OrderNodeType["data"]) ?? {}

      const iconPath = `/api/v1/connValueType-icon?key=orders`
