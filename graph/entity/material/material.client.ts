...
import type { MaterialNodeType } from "./material.type"

export const createMaterialEntity: EntityCreator = () => {
  const imageCache = new Map<string, HTMLImageElement>()

  return {
    renderNodeCanvasObject: ({ node, ctx, globalScale, style }) => {
      const { x = 0, y = 0 } = node
      const { label, materialName, pageIndex, pageSize, count } =
        (node.data as MaterialNodeType["data"]) ?? {}

      const iconPath = `/api/v1/connValueType-icon?key=material`
